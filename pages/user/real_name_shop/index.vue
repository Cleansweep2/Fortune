		...
		<view class="content">
			<view class="content_top">
				<view class="row">
					...
					<view ckass="right">
						<input :value="store_name" @input="changInput($event,1)" placeholder="请输入店铺简称" />
					</view>
				</view>
				<view class="row">
					<view class="left">联系方式</view>
					<view ckass="right">
						<input :value="store_user_phone" @input="changInput($event,2)" placeholder="请输入联系电话" />
					</view>
				</view>
				<view class="row">
					<view class="left">联系人</view>
					<view ckass="right">
						<input :value="store_user_name" @input="changInput($event,3)" placeholder="请输入联系人名称" />
					</view>  
				</view>
				<view class="row">
					<view class="left">商家地址</view>
					<view ckass="right">
						<input :value="store_address" @input="changInput($event,5)" placeholder="x省 x市 x区 x街 x号" />
					</view>
				</view>
			</view>
		...
		methods: {
			...
			async getShopDetail(){
				const res = await this.$http({
					url:'/store/storeAuthentication'
				})
				const data = res[1].data
				if(data.code == 200){
					const  goods_detail = data.data
					this.img1 = goods_detail.image1_url
					this.lat = goods_detail.lat
					this.lng =  goods_detail.lng 
					this.store_address = goods_detail.store_address
					this.img = goods_detail.store_logo_url
					this.store_name = goods_detail.store_name
					this.store_user_name = goods_detail.store_user_name
					this.store_user_phone = goods_detail.store_user_phone
				}
			},
			changInput(e, i) {
