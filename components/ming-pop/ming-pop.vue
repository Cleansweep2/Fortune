<template>
  ...
  <view :class="direction==='center'?'centers':''" v-if="direction==='center'?open:true">
		<view class="product-window" :style="{width:width+'%'}" :class="(open ? 'on' : '')+' '+direction">
			...
			<!-- #endif -->
			<image src="../../static/images/ming-pop/close.png" mode="" :class="(direction!=='below'&&direction!=='center')?'iconfont-h5':''" class="iconfont" @click="open=false" v-if="is_close"></image>
			<slot></slot>
		</view>
